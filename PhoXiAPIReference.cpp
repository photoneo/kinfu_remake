...
int main(int argc, char *argv[]) {
	...
	/*SimpleNormalsEstimator::HowToUse();
	...
	while (true) {
		pho::api::PhoXiCoordinatesSettings Settings = Device->CoordinatesSettings;
		Device->CoordinatesSettings->RecognizeMarkers = !Settings.RecognizeMarkers;
		Device->CoordinatesSettings->MarkersSettings.InvertedMarkers = !Settings.MarkersSettings.InvertedMarkers;
		if (Settings.CoordinateSpace == pho::api::PhoXiCoordinateSpace::CustomSpace) {
			Device->CoordinatesSettings->CoordinateSpace = pho::api::PhoXiCoordinateSpace::CameraSpace;
		}
		else {
			Device->CoordinatesSettings->CoordinateSpace = pho::api::PhoXiCoordinateSpace::CustomSpace;
		}
		Device->CoordinatesSettings->CustomTransformation.Translation.x = Settings.CustomTransformation.Translation.x + 1.0;
		Device->CoordinatesSettings->CustomTransformation.Rotation[1][1] = Settings.CustomTransformation.Rotation[1][1] + 1.0;
	}


	//for (;;) RecodeTest();
	/*{
	pho::PointCloud TempCloud = pho::Plyio::plyRead("C:\\Users\\tomas\\Downloads\\Downloads2\\badone.ply");
	cv::imwrite("C:\\Users\\tomas\\Downloads\\Downloads2\\badone.png", TempCloud.colors);
	}
	{
	pho::PointCloud TempCloud = pho::Plyio::plyRead("C:\\Users\\tomas\\Downloads\\Downloads2\\goodone.ply");
	cv::imwrite("C:\\Users\\tomas\\Downloads\\Downloads2\\goodone.png", TempCloud.colors);;
	}*/

#if 0
	//Factory.MinimizePhoXiControl();
	if (0) {
		std::stringstream Stream;
		boost::archive::binary_oarchive OArchive(Stream);
		EasyToSerialize Instance, Instance2;

		Instance.TakedoTaco = 45;
		Instance.AjToto = 69;
		Instance.MoznoTakto = 42;
		Instance.NiecoIne = -451221.32181618;
		Instance.ATak = 0.0f;
		OArchive << Instance;

		pho::Interprocess::HelperFunctions::SerializableObjectWrapper<EasyToSerialize> Wrapper(Instance);
		Wrapper.Serialize/*<EasyToSerialize>*/();

		Wrapper.Deserialize/*<EasyToSerialize>*/();
		boost::archive::binary_iarchive IArchive(Stream);
		int Haluz = sizeof(EasyToSerialize);
		std::string BinaryData = Stream.str();
		std::string BinaryDataDirect;
		BinaryDataDirect.resize(Haluz);
		memcpy(&BinaryDataDirect[0], &Instance, Haluz);
	...
	if (1) {
		boost::this_thread::sleep_for(boost::chrono::milliseconds(5000));
		//std::string DeviceSerial = "20160324001";
		std::string DeviceSerial = "20160614001";
		if (argc > 1) DeviceSerial = argv[1];
		std::vector<double> SupportedSweepUpTimes;
		SupportedSweepUpTimes.push_back(10.240);
		SupportedSweepUpTimes.push_back(14.336);
		SupportedSweepUpTimes.push_back(20.480);
		SupportedSweepUpTimes.push_back(24.576);
		SupportedSweepUpTimes.push_back(30.720);
		SupportedSweepUpTimes.push_back(34.816);
		SupportedSweepUpTimes.push_back(40.960);
		SupportedSweepUpTimes.push_back(49.152);
		SupportedSweepUpTimes.push_back(75.776);
		SupportedSweepUpTimes.push_back(79.872);
		SupportedSweepUpTimes.push_back(90.112);
		SupportedSweepUpTimes.push_back(100.352);

		std::vector <std::string> ImageFilePaths;
		/*ImageFilePaths.push_back("C:/Work/Scanners/PhoXi3DScan-10671028/2DM-20m/Actions/PhaseShift/0/Debug/index.Mat.tif");
		ImageFilePaths.push_back("C:/Work/Scanners/PhoXi3DScan-10671028/5DM-100ms/Actions/PhaseShift/0/Debug/index.Mat.tif");*/
		/*ImageFilePaths.push_back("C:/Work/Scanners/EuclidUpdate20160408/index.Mat20.tif");
		ImageFilePaths.push_back("C:/Work/Scanners/EuclidUpdate20160408/index.Mat100.tif");*/

		CalibrateOffsetShifts(SupportedSweepUpTimes, DeviceSerial, ImageFilePaths, 0);
		return 0;
	}
#if 0
	if (0) {
		CameraCalibrateModeExample();
		return 0;
	}
	if (0) {
		IPConnectExample();
	...
	//boost::this_thread::sleep_for(boost::chrono::milliseconds(5000));
	pho::PFastDelayedConsole.startLocal();
	//const int TestCount = 3;
	//std::vector<pho::CameraCalibration64> TransformCalibration(TestCount);
	//std::vector<pho::CameraCalibration64> TransformCalibrationOutput(TestCount);
	//std::vector<std::vector<cv::Point2d>> ImagePointSets(TestCount);
	//std::vector<std::vector<cv::Point3d>> WorldPointSets(TestCount);

	//std::vector<pho::CameraCalibration32> TransformCalibration32(TestCount);
	//std::vector<pho::CameraCalibration32> TransformCalibrationOutput32(TestCount);
	//std::vector<std::vector<cv::Point2f>> ImagePointSets32(TestCount);
	//std::vector<std::vector<cv::Point3f>> WorldPointSets32(TestCount);

	//for (std::size_t i = 0; i < TransformCalibration.size(); i++) {
	//    TransformCalibration[i].LoadFromFile("C:/Users/tomas/Desktop/Temp/MarkersDaco/0TransformationCalibration" + std::to_string(i) + ".xml");
	//    TransformCalibration32[i] = TransformCalibration[i];
	//    cv::FileStorage fs("C:/Users/tomas/Desktop/Temp/MarkersDaco/0WorldImage" + std::to_string(i) + ".xml", cv::FileStorage::Mode::READ);
	//    if (fs.isOpened()) {
	//        fs["WorldCoordinates"] >> WorldPointSets[i];
	//        fs["ImagePoints"] >> ImagePointSets[i];

	//        fs["WorldCoordinates"] >> WorldPointSets32[i];
	//        fs["ImagePoints"] >> ImagePointSets32[i];
	//    }
	//    if (i == 1) {
	//        /*WorldPointSets[i].erase(WorldPointSets[i].begin() + 2);
	//        ImagePointSets[i].erase(ImagePointSets[i].begin() + 2);*/
	//    }

	//    for (std::size_t k = 0; k < WorldPointSets[i].size(); k++) {
	//        //WorldPointSets[i][k].y *= -1;
	//    }
	//    fs.release();
	//    phoc << TransformCalibration[i].GetCameraToWorldCoordinates() << std::endl;

	//    cv::Mat RVec = cv::Mat(3, 1, cv::DataType<double>::type);
	//    cv::Mat TVec = cv::Mat(3, 1, cv::DataType<double>::type);
	//    cv::Mat RVec32 = cv::Mat(3, 1, cv::DataType<float>::type);
	//    cv::Mat TVec32 = cv::Mat(3, 1, cv::DataType<float>::type);

	//    std::vector<int> consensusPoints;

	//    double ReprojectionError = SolvePnPRansac(WorldPointSets[i], ImagePointSets[i], TransformCalibration[i], TransformCalibrationOutput[i], consensusPoints);
	//    //;
	//    //if (cv::solvePnP(WorldPointSets[i], ImagePointSets[i], TransformCalibration[i].GetCameraMatrix(), TransformCalibration[i].GetDistortionCoefficients(), RVec, TVec)) {
	//    //if (cv::solvePnPRansac(WorldPointSets[i], ImagePointSets[i], TransformCalibration[i].GetCameraMatrix(), TransformCalibration[i].GetDistortionCoefficients(), RVec, TVec, false, 100, 80000.0f, 0.99, consensusPoints)) {
	//    //    //if (consensusPoints.size() >= 4) {
	//    //    phoc << RVec << std::endl;
	//    //    phoc << TVec << std::endl;
	//    //    TransformCalibrationOutput[i] = TransformCalibration[i];
	//    //    //TransformCalibrationOutput[i].SetCameraToWorldCoordinates(RVec, TVec);
	//    //    TransformCalibrationOutput[i].SetWorldToCameraCoordinates(RVec, TVec);
	//    //    //} else {
	//    //    //}
	//    //} else {
	//    //}

	//    //if (cv::solvePnPRansac(WorldPointSets32[i], ImagePointSets32[i], TransformCalibration32[i].GetCameraMatrix(), TransformCalibration32[i].GetDistortionCoefficients(), RVec32, TVec32, false, 1000, 8.0f, 0.9999, consensusPoints)) {
	//    //    //if (consensusPoints.size() >= 4) {
	//    //    TransformCalibrationOutput32[i] = TransformCalibration32[i];
	//    //    TransformCalibrationOutput32[i].SetWorldToCameraCoordinates(RVec32, TVec32);
	//    //    //} else {
	//    //    //}
	//    //} else {
	//    //}
	//    std::vector<cv::Point2d> LocalImagePoints(WorldPointSets[i].size());
	//    TransformCalibrationOutput[i].Project3DPointsToImagePoints(WorldPointSets[i], LocalImagePoints);
	//    for (std::size_t k = 0; k < LocalImagePoints.size(); k++) {
	//        phoc << (LocalImagePoints[k] - ImagePointSets[i][k]) << std::endl;
	//    }


	//    phoc << TransformCalibrationOutput[i].GetCameraToWorldCoordinates() << std::endl;
	//    //phoc << TransformCalibrationOutput32[i].GetCameraToWorldCoordinates() << std::endl;
	//    phoc << std::endl << std::endl << std::endl;
	//}
	//return 0;
#endif
	pho::PFastDelayedConsole.startLocal();
	...
	if (0) {
...

		{
			...
			std::ifstream ICPCoordinatesStream("C:/Users/tomas/Desktop/CalibVerification/Robot-Camera-Calibration-Data/CoordinatesICP.txt");
			std::ifstream RobotCoordinatesStream("C:/Users/tomas/Desktop/CalibVerification/Robot-Camera-Calibration-Data/calibrationData.txt");

			std::vector<cv::Point3d> ICPCoordinates;
			std::vector<pho::Transformation3D64> RobotTransformations;

			std::vector<std::vector<cv::Point3d>> InlierPoints;

			int Count = 48;// 36;
			int Index = 0;


			while (!ICPCoordinatesStream.eof() && !RobotCoordinatesStream.eof() && Index < Count) {
				cv::Point3d ActualPoint, ActualRotation;
				ICPCoordinatesStream >> ActualPoint.x;
				ICPCoordinatesStream >> ActualPoint.y;
				ICPCoordinatesStream >> ActualPoint.z;
				//ActualPoint.z *= -1.0;
				ICPCoordinates.push_back(ActualPoint /** -1.0*/);

				pho::PointCloud Sphere = pho::plyRead(phoc.ToString("C:/Users/tomas/Desktop/CalibVerification/Robot-Camera-Calibration-Data/Inliers#.ply", Index));

				cv::Point3d LocalSphereCenter(0.0, 0.0, 0.0);
				std::vector<cv::Point3d> LocalSphere;
				for (int y = 0; y < Sphere.points.rows; ++y) {
					for (int x = 0; x < Sphere.points.cols; ++x) {
						LocalSphere.push_back((cv::Point3d)Sphere.points.at<cv::Point3f>(y, x));
						LocalSphereCenter += LocalSphere.back();
					}
				}
				InlierPoints.push_back(LocalSphere);
				LocalSphereCenter *= 1.0 / (double)LocalSphere.size();
				double Radius = 20.3;

				//pho::RobotCalibration::CeresSphereFitter::FitSphereWithRadius(LocalSphere, LocalSphereCenter, Radius);
				phoc("Radius: #", Radius);
				ICPCoordinates.back() = LocalSphereCenter;



				RobotCoordinatesStream >> ActualPoint.x;
				RobotCoordinatesStream >> ActualPoint.y;
				RobotCoordinatesStream >> ActualPoint.z;

				RobotCoordinatesStream >> ActualRotation.x;
				RobotCoordinatesStream >> ActualRotation.y;
				RobotCoordinatesStream >> ActualRotation.z;

				RobotTransformations.push_back(pho::Transformation3D64(pho::RotationVector64(ActualRotation), pho::Translation3D64(ActualPoint)));
				++Index;
			}
			//pho::RobotCalibration::DoCalibrationTestFunction(RobotTransformations, ICPCoordinates);

			double Radius = 19.8;
			pho::RobotCalibration::DoCalibrationTestFunction(RobotTransformations, InlierPoints, Radius, true);
			/*cv::Point3d SphereCenterInTCPCoordinates (0.0, 0.0, 0.0);
			pho::Transformation3D64 FinalTransformation;
			Volumetric::FindRobotScannerTransformation(RobotTransformations, ICPCoordinates, FinalTransformation, SphereCenterInTCPCoordinates);*/
		}
		...
		std::ifstream ICPCoordinatesStream("C:/Users/tomas/Desktop/CalibVerification/warpfield2/CoordinatesICP.txt");
		std::ifstream RobotCoordinatesStream("C:/Users/tomas/Desktop/CalibVerification/warpfield2/positions.txt");

		std::vector<cv::Point3d> RobotCoordinates, ICPCoordinates, TransformedICPCoordinates, Differences;
		int Count = 616;// 36;
		int Index = 0;
		...
		while (!ICPCoordinatesStream.eof() && !RobotCoordinatesStream.eof() && Index < Count) {
			cv::Point3d ActualPoint;
			ICPCoordinatesStream >> ActualPoint.x;
			ICPCoordinatesStream >> ActualPoint.y;
			ICPCoordinatesStream >> ActualPoint.z;
			ICPCoordinates.push_back(ActualPoint);

			RobotCoordinatesStream >> ActualPoint.x;
			RobotCoordinatesStream >> ActualPoint.y;
			RobotCoordinatesStream >> ActualPoint.z;
			RobotCoordinates.push_back(ActualPoint * 1000.0);

			++Index;
		}

		cv::Mat Transformation;
		photoneoTools::CameraCalibration64 TransformationCalibration;
		photoneoTools::GetTransformationFromTo3D(ICPCoordinates, RobotCoordinates, Transformation);
		TransformationCalibration.SetCameraToWorldCoordinates(Transformation(cv::Rect(0,
			0,
			4,
			3)).clone());


		TransformedICPCoordinates.resize(ICPCoordinates.size());
		Differences.resize(ICPCoordinates.size());
		TransformationCalibration.ConvertToWorldCoordinates(ICPCoordinates, TransformedICPCoordinates);

		std::ofstream OutputDiffStream("Output/OutputDifferences.txt");

		for (std::size_t i = 0; i < TransformedICPCoordinates.size(); i++) {
			Differences[i] = TransformedICPCoordinates[i] - RobotCoordinates[i];
			OutputDiffStream << cv::norm(Differences[i]);
			if (i % xSize == xSize - 1) {
				if (i % xSize * ySize == xSize * ySize - 1) {
					OutputDiffStream << std::endl;
				}
				OutputDiffStream << std::endl;
			}
			else {
		...
		OutputDiffStream.close();


		std::vector<cv::Point3f> Sphere;

		double Radius = 5.0;
		for (double HorizontalAngle = 0.0; HorizontalAngle <= 2.0 * CV_PI; HorizontalAngle += CV_PI / 36.0) {
			for (double VerticalAngle = 0.0; VerticalAngle <= CV_PI; VerticalAngle += CV_PI / 72.0) {
				cv::Point3d CirclePoint, OriginalCirclePoint;
				CirclePoint.z = std::cos(VerticalAngle);
				double Temp = std::sin(VerticalAngle);
				CirclePoint.x = std::cos(HorizontalAngle) * Temp;
				CirclePoint.y = std::sin(HorizontalAngle) * Temp;
				CirclePoint *= Radius;
				Sphere.push_back((cv::Point3f)CirclePoint);
			}
		}

		cv::Mat Robot(cv::Size(Sphere.size(), RobotCoordinates.size()), CV_32FC3, cv::Scalar(0.0, 0.0, 0.0));
		cv::Mat RobotTexture(cv::Size(Sphere.size(), RobotCoordinates.size()), CV_8UC3, cv::Scalar(0.0, 255.0, 0.0));
		cv::Mat ICPTransform(cv::Size(Sphere.size(), TransformedICPCoordinates.size()), CV_32FC3, cv::Scalar(0.0, 0.0, 0.0));
		cv::Mat ICPTransformTexture(cv::Size(Sphere.size(), TransformedICPCoordinates.size()), CV_8UC3, cv::Scalar(0.0, 0.0, 255.0));
		...
		pho::PointCloud RobotCloud(Robot, RobotTexture);
		pho::PointCloud ICPTransformCloud(ICPTransform, ICPTransformTexture);

		pho::Plyio::plyWrite("Output/CAL-RobotCoordinates.ply", RobotCloud);
		pho::Plyio::plyWrite("Output/CAL-ICPTransformCoordinates.ply", ICPTransformCloud);
		...
		//!!!!!
		TransformedICPCoordinates = ICPCoordinates;


		double XMin = TransformedICPCoordinates[0].x;
		double YMin = TransformedICPCoordinates[0].y;
		double ZMin = TransformedICPCoordinates[0].z;
		double XMax = TransformedICPCoordinates[0].x;
		double YMax = TransformedICPCoordinates[0].y;
		double ZMax = TransformedICPCoordinates[0].z;
		for (std::size_t i = 0; i < TransformedICPCoordinates.size(); i++) {
			XMin = std::min(XMin, TransformedICPCoordinates[i].x);
			YMin = std::min(YMin, TransformedICPCoordinates[i].y);
			ZMin = std::min(ZMin, TransformedICPCoordinates[i].z);
			XMax = std::max(XMax, TransformedICPCoordinates[i].x);
			YMax = std::max(YMax, TransformedICPCoordinates[i].y);
			ZMax = std::max(ZMax, TransformedICPCoordinates[i].z);
		}
		XMin -= Offset;
		YMin -= Offset;
		ZMin -= Offset;
		...
		int BenchMarkPointCount = 3200000;
		std::vector<cv::Point3f> BenchMarkPointCloudExact(BenchMarkPointCount);
		cv::Point3f ActualPoint;

		pho::Volumetric::VolumeWarperPrism32 TempPrism(XMin, YMin, ZMin, XMax, YMax, ZMax);

		ActualPoint.x = (double)(std::rand() % 10001) / 10000.0;
		ActualPoint.y = (double)(std::rand() % 10001) / 10000.0;
		ActualPoint.z = (double)(std::rand() % 10001) / 10000.0;
		for (std::size_t i = 0; i < BenchMarkPointCloudExact.size(); i++) {
			ActualPoint.x = (double)(std::rand() % 10001) / 10000.0;
			ActualPoint.y = (double)(std::rand() % 10001) / 10000.0;
			ActualPoint.z = (double)(std::rand() % 10001) / 10000.0;

			BenchMarkPointCloudExact[i] = TempPrism.ToWorldCoordinates(ActualPoint);
		}

		std::vector<cv::Point3f> BenchMarkPointCloudAccelerated = BenchMarkPointCloudExact;
		std::vector<cv::Point3f> BenchMarkPointCloudAcceleratedSingle = BenchMarkPointCloudExact;
		std::vector<cv::Point3f> BenchMarkPointCloudOriginal = BenchMarkPointCloudExact;


		pho::Volumetric::BezierVolumeWarperPrism32 BezierVolumePrism(XMin, YMin, ZMin, XMax, YMax, ZMax);
		double InitialReprojectionError, FinalReprojectionError, InitialReprojectionErrorMSQR, FinalReprojectionErrorMSQR;

		double FitDuration, CreateAccelaratedStructureDuration, WarpBezier, WarpAccelerated, WarpAcceleratedMulti;
		CDuration Duration;

		Duration.Start();
		BezierVolumePrism.FitVolume(RobotCoordinates, TransformedICPCoordinates, Order, InitialReprojectionError, FinalReprojectionError, InitialReprojectionErrorMSQR, FinalReprojectionErrorMSQR);
		Duration.Stop(); FitDuration = Duration.GetDuration();

		Duration.Start();
		BezierVolumePrism.CreateAcceleratedStructure(10.0f);
		Duration.Stop(); CreateAccelaratedStructureDuration = Duration.GetDuration();
		phoc << phoc.ToString("Initial reprojection error: # , MSQR # --> Final reprojection error: #, MSQR #", InitialReprojectionError, InitialReprojectionErrorMSQR, FinalReprojectionError, FinalReprojectionErrorMSQR) << std::endl;


		Duration.Start();
		//BezierVolumePrism.WarpPoints(BenchMarkPointCloudExact);
		Duration.Stop(); WarpBezier = Duration.GetDuration();

		Duration.Start();
		BezierVolumePrism.WarpPointsAcceleratedInVector(BenchMarkPointCloudAcceleratedSingle, 1);
		Duration.Stop(); WarpAccelerated = Duration.GetDuration();

		Duration.Start();
		BezierVolumePrism.WarpPointsAcceleratedInVector(BenchMarkPointCloudAccelerated);
		Duration.Stop(); WarpAcceleratedMulti = Duration.GetDuration();

		double DiffSum = 0.0;
		double MaximalError = -1.0;

		for (std::size_t i = 0; i < BenchMarkPointCloudExact.size(); i++) {
			/*cv::Point3f Point0 = BenchMarkPointCloudExact[i];
			cv::Point3f Point1 = BezierVolumePrism.WarpPoint(Point0);
			cv::Point3f Point2 = BezierVolumePrism.UnWarpPoint(Point1);
			cv::Point3f Point3 = Point2 - Point0;*/

			double ActualDistance = cv::norm((cv::Point3d)BenchMarkPointCloudExact[i] - (cv::Point3d)BenchMarkPointCloudAccelerated[i]);
			if (MaximalError < 0.0 || ActualDistance > MaximalError) {
				MaximalError = ActualDistance;
			}
			DiffSum += ActualDistance;
		}

		DiffSum /= (double)BenchMarkPointCloudExact.size();


		phoc << phoc.ToString("It took # to Fit, # to create Accelerated Structure. # to WarpBezier, # to Warp Accelerated, # to Warp Accemerated in multiple threads", FitDuration, CreateAccelaratedStructureDuration, WarpBezier, WarpAccelerated, WarpAcceleratedMulti) << std::endl;

		phoc << phoc.ToString("Maximal Distance with Accelerated: #, Average Distance: #", MaximalError, DiffSum) << std::endl;

		//BezierVolumeWarp::BezierVolumeDiffFitter<Order, Order, Order> DiffFitter(RobotCoordinates, TransformedICPCoordinates);
	...
	/*{
	pho::api::PhoXiFactory Factory;
	pho::api::PPhoXi PhoXi = Factory.CreateAndConnectFirstAttached();

	pho::SimpleLinearAxis Axis(5);

	bool Opened = Axis.isInitiated();
	if (Opened) {
	pho::api::PhoXiRawAccessHandler RawHandler(PhoXi);
	for (double Distance = 870.0; Distance < 2156; Distance += 10.0) {
	Axis.GoToPosition(2140.0 - (Distance - 390.0));
	while (Axis.isBusy()) {
	boost::this_thread::sleep_for(boost::chrono::milliseconds(100));
	}
	boost::this_thread::sleep_for(boost::chrono::milliseconds(1000));
	pho::api::PFrame Frame = PhoXi->GetSpecificFrame(PhoXi->TriggerFrame(true, true));
	pho::PDataManager Manager = RawHandler.GetLastOutput();
	cv::Mat RawImage = Manager->GetLeaf("Global/Cameras/0/RawImagesMat/Image0.Mat");
	cv::imwrite(phoc.ToString("Output/TestIntensityDist(#).png", (int)Distance + 10000), RawImage);
	...
	/*google::InitGoogleLogging(argv[0]);
	pho::CameraCalibration::CeresMultipleCamerasCalibrationTest();*/
	...
	{
		//std::string DeviceSerial = "20160324001";
		std::string DeviceSerial = "20160412001";
		if (argc > 1) DeviceSerial = argv[1];
		std::vector<double> SupportedSweepUpTimes;
		SupportedSweepUpTimes.push_back(10.240);
		SupportedSweepUpTimes.push_back(14.336);
