...
bool kfusion::cuda::ProjectiveICP::estimateTransform(Affine3f& affine, const Intr& intr, const DepthPyr& dcurr, const NormalsPyr ncurr, const DepthPyr dprev, const NormalsPyr nprev)
{
    ...
    Affine3f input_transformation = affine;
    float treshold = 0.001;
    float treshold_rot = 0.998;
    ...
    for(int level_index = LEVELS - 1; level_index >= 0; --level_index)
    {
        ...
        for(int iter = 0; iter < iters_[level_index]; ++iter)
        {
            ...
            Affine3f good_affine = affine;
            affine = Tinc * affine;

            Affine3f rotinv = input_transformation.inv() * affine;
            std::cout << "MATICA XX" << std::endl;

            Mat3f rot=rotinv.rotation();
            std::cout << rot(0,0) << std::endl;
            std::cout << rot(1,1) << std::endl;
            std::cout << rot(2,2) << std::endl;

            if(rot(0,0) < treshold_rot || rot(1,1) < treshold_rot || rot(2,2) < treshold_rot){
                affine = good_affine;
                return true;
            }

            if (fabs(fabs(affine.translation()[2]) - fabs(input_transformation.translation()[2])) > treshold || fabs(fabs(affine.translation()[0]) - fabs(input_transformation.translation()[0])) > treshold || fabs(fabs(affine.translation()[1]) - fabs(input_transformation.translation()[1])) > treshold) {
                affine = good_affine;
                return true;
            }
