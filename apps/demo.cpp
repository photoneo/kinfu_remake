...
/*---------------------------------------------------------------------------------------*/
    ...
    bool read_transformation_file(const char* path,cv::Affine3f &transformation){
       ...
       /* Mat3f rotxx=previous_rot * rot.inv();


        std::cout << "MATICAXX" << std::endl;
        std::cout << rotxx(0,0) << std::endl;
        std::cout << rotxx(1,1) << std::endl;
        std::cout << rotxx(2,2) << std::endl;
        if (rotxx(0,0) < 0.996 || rotxx(1,1) < 0.996 || rotxx(2,2) < 0.996){
            previous_rot = rot;
    ...
    bool execute()
    {
        ...
        for (int i = 1; !exit_ && !viz.wasStopped(); ++i) {
            ...
            if (index != 0) {
                ...
                if(read_transformation_file(path.c_str(), transformation));
            }
            index++;
